...
import ModalGuild from "./ModalGuild";
...
export default function ListGuilds() {
  ...
  const [guildForm, setGuildForm] = useState<IGuild | null>(null);
  const [guildData, setGuildData] = useState<IGuild[]>([]);
  const [guildList, setGuildList] = useState<IGuild[]>([]);
  ...
  useEffect(() => {
      ...
      guildService.list()
        .then((r) => {
          setGuildData(r.data);
          setGuildList(r.data.map((d: IGuild) => ({
            id: d.id,
            name: d.name
          })));
  ...
  useEffect(() => {
    if (openModal === false) setGuildForm(null)
  }, [openModal]);

  const openModalFunc = (item: IGuild) => {
    const u = guildData.find(d => d.id === item.id) as IGuild;
    if (u) {
      setGuildForm({
        ...u
      });
  ...
  const deleteItem = (item: IGuild) => {
    alertSwal.fire({
      ...
      if (result.isConfirmed) {
        guildService.destroy(item.id as number)
          .then(() => {   
            alertSwal.fire({
              title: "Excluído!",
  ...
  const renderActionBtn = (item: IGuild) => {
    return (
      <div className="space-x-1">
        <button 
          title="Editar operação."
          onClick={() => item.name && openModalFunc(item as unknown as IGuild)}
  ...
  return (
      <div>
        <section>
            ...
            <div className="flex justify-center">
              { loading ? <LoadCircle /> :
                <div className="relative h-full w-full rounded bg-white shadow">
                    ...
                    <form action="" method="get" className="w-full">
                      {/* <div className="relative">
                        ...
                        <div className="flex absolute end-2.5 bottom-2.5 space-x-2">
                          ...
                          { filter && 
                            <a href="/guilds" className="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2">
                  ...
                  <Table headers={headers} data={guildList} renderAction={renderActionBtn} />
          ...
          { openModal && <ModalGuild guild={guildForm} setOpenModal={setOpenModal} /> }
        </section>
      </div>
  );
}